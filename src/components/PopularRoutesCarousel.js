...
export default function PopularRoutesCarousel({ routes = defaultRoutes }) {
    ...
    const getGoogleMapsUrl = (placeName) => {
        if (!placeName) return "https://www.google.com/maps";
        // Encode the place name for URL safety
        ...
        <div className="relative">
            ...
            <div className="pointer-events-none absolute inset-0 flex items-center">
                <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-16 xl:px-24">
                    <div
                        className={`max-w-2xl pr-2 sm:pr-4 md:pr-8 lg:pr-12 transition-all duration-500 ease-out will-change-transform will-change-opacity ${
                            textAnimating
                    ...
                    >
                        <h3 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white drop-shadow mb-3 sm:mb-4">
                            {routes[selectedIndex].title ||
                                `${routes[selectedIndex].name}`}
                        </h3>
                    ...
                    <div className="pointer-events-auto mt-5 sm:mt-6">
                        <Link
                            href={getGoogleMapsUrl(routes[selectedIndex].name)}
                            target="_blank"
            ...
            <div className="absolute bottom-5 right-5 sm:bottom-7 sm:right-7 md:bottom-10 md:right-10 lg:bottom-12 lg:right-12 flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex items-center gap-2">
                    <button
                        onClick={scrollPrev}
                        className="w-9 h-9 sm:w-10 sm:h-10 md:w-11 md:h-11 rounded-full border border-white/60 text-white flex items-center justify-center hover:bg-white/20 transition"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-5 w-5"
