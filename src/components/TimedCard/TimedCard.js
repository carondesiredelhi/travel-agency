...
export default function TimedCards({
    ...
    useEffect(() => {
        ...
        const loadImages = async () => {
            try {
                ...
                await Promise.all(
                    items.map(({ image }, idx) => loadImage(image, idx))
                );
    ...
    useEffect(() => {
        ...
        const updateLayout = () => {
            if (!isLoaded || !items?.length) return;

            const isMobile = window.innerWidth < 768;
            const isTablet =
                window.innerWidth < 1024 && window.innerWidth >= 768;

            const cardWidth = isMobile ? 140 : isTablet ? 170 : 200;
            const cardHeight = isMobile ? 220 : isTablet ? 260 : 300;
            const gap = isMobile ? 20 : isTablet ? 30 : 40;
            const numberSize = isMobile ? 36 : isTablet ? 44 : 50;
            const ease = "sine.inOut";

            let offsetTop =
                window.innerHeight - (isMobile ? 300 : isTablet ? 380 : 430);
            let offsetLeft =
                window.innerWidth - (isMobile ? 400 : isTablet ? 600 : 830);

            const init = () => {
                const [active, ...rest] = order.current;
                const detailsActive = detailsEven.current
                    ? detailsEvenRef.current
                    : detailsOddRef.current;
                const detailsInactive = detailsEven.current
                    ? detailsOddRef.current
                    : detailsEvenRef.current;
                ...
                gsap.set(cardContentRefs.current[active], {
                    x: 0,
                    y: 0,
                    opacity: 0,
                });
                ...
                [".text", ".title-1", ".title-2"].forEach((sel) =>
                    gsap.set(detailsInactive.querySelectorAll(sel), { y: 100 })
                );
                ...
                if (progressSubForegroundRef.current) {
                    gsap.set(progressSubForegroundRef.current, {
                        width: `${
                            (100 / order.current.length) * (active + 1)
                        }%`,
                    });
                }
                ...
                rest.forEach((i, index) => {
                    ...
                    gsap.set(slideItemRefs.current[i], {
                ...
                if (activeOverlayRef.current) {
                    gsap.to(activeOverlayRef.current, {
                        opacity: 1,
                        ease,
                        delay: startDelay,
                    });
            ...
            const step = (direction = 1) => {
                ...
                gsap.timeline({
                    ...
                    .to(
                        detailsActive.querySelectorAll(
                            ".text, .title-1, .title-2"
                        ),
                        { y: 0, duration: 0.7, ease, stagger: 0.1 },
                        0.1
                    )
                    .to(
                        detailsActive.querySelector(".desc"),
                        { y: 0, duration: 0.4, ease },
                        0.3
                    )
                    .to(
                        detailsActive.querySelector(".cta"),
                        { y: 0, duration: 0.4, ease },
                        0.35
                    )
                    ...
                    .to(
                        cardContentRefs.current[active],
                        {
                    ...
                    .to(
                        progressSubForegroundRef.current,
                        {
                            width: `${
                                (100 / order.current.length) * (active + 1)
                            }%`,
                    ...
                    .to(
                        cardRefs.current[active],
                        {
                            ...
                            onComplete: () => {
                                ...
                                gsap.set(slideItemRefs.current[prv], {
                                    x: rest.length * numberSize,
                                });
                                gsap.set(detailsInactive, { opacity: 0 });
                                gsap.set(
                                    detailsInactive.querySelectorAll(
                                        ".text, .title-1, .title-2"
                                    ),
                                    { y: 100 }
                                );
                                gsap.set(
                                    detailsInactive.querySelector(".desc"),
                                    { y: 50 }
                                );
                                gsap.set(
                                    detailsInactive.querySelector(".cta"),
                                    { y: 60 }
                                );
                                if (activeOverlayRef.current) {
                    ...
                    .to(
                        ...
                        {
                            x: (i, target) => {
                                const originalIndex = order.current.indexOf(
                                    parseInt(target.id.replace("card", ""))
                                );
                                return (
                                    offsetLeft +
                                    (originalIndex - 1) * (cardWidth + gap)
                                );
                            },
                            ease,
                            delay: 0.1,
                    ...
                    .to(
                        rest
                            .filter((i) => i !== prv)
                            .map((i) => cardContentRefs.current[i]),
                        {
                            x: (i, target) => {
                                const originalIndex = order.current.indexOf(
                                    parseInt(
                                        target.id.replace("card-content-", "")
                                    )
                                );
                                return (
                                    offsetLeft +
    ...
    const Details = ({ item, ...props }) => {
        ...
        const getGoogleMapsUrl = (place) => {
            const mapsUrl = {
                Uttarakhand:
                    "https://maps.google.com/maps?q=Uttarakhand,+India",
                "Himachal Pradesh":
                    "https://maps.google.com/maps?q=Himachal+Pradesh,+India",
                Rajasthan: "https://maps.google.com/maps?q=Rajasthan,+India",
                "Uttar Pradesh":
                    "https://maps.google.com/maps?q=Uttar+Pradesh,+India",
        ...
        return (
            <div className={styles.details} {...props}>
                {/* <div className={styles.placeBox}>
                    <div className={`${styles.text} text`}>{item.place}</div>
                </div> */}
                <div className={`${styles.title1} title-1`}>{item.title}</div>
                <div className={`${styles.title2} title-2`}>{item.title2}</div>
                ...
                <div className={`${styles.cta} cta`}>
                    ...
                    <Link
                        href={getRouteUrl(item.place)}
                        className={`${styles.discover} flex items-center`}
                    >
                        Discover Location
                    </Link>
                </div>
                ...
                {items.map((item, index) => (
                    <div key={`${item.title}-${index}`}>
                        <div
                            ...
                            style={{
                                backgroundImage: imageLoaded[index]
                                    ? `url(${item.image})`
                                    : "none",
                                backgroundColor: imageLoaded[index]
                                    ? undefined
                                    : "#222",
                                transition:
                                    "background-image 0.3s, background-color 0.3s",
                            }}
                        >
                        ...
                        >
                            ...
                            <div className={styles.contentTitle1}>
                                {item.title}
                            </div>
